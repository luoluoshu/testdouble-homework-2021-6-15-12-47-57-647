...
class PrinterTest {
    ...
    void should_not_change_given_transactions_when_call_print() {
        ...
        Transaction transaction2 = new Transaction("16/06/2022", 1);
    ...
    void should_print_with_separator_between_when_call_statementLine_given_date_amount_balance() {
        Console dummyConsole = mock(Console.class);
        Printer printer = new Printer(dummyConsole);
        String result = printer.statementLine(new Transaction("16/06/2021", 1), 2);
        assertEquals("16/06/2021 | 1 | 2", result);
    }
    @Test
    void should_print_with_date_desc_when_call_print_given_date_casual_order() {
        Console spyConsole = mock(Console.class);
        Printer printer = new Printer(spyConsole);
        Transaction transaction1 = new Transaction("16/06/2021", 0);
        Transaction transaction2 = new Transaction("16/06/2022", 0);
        Transaction transaction3 = new Transaction("16/06/2023", 0);
        List<Transaction> givenTransactions = Arrays.asList(transaction1, transaction2, transaction3);
        printer.print(givenTransactions);
        verify(spyConsole, times(1)).printLine("16/06/2021 | 0 | 0");
        verify(spyConsole, times(1)).printLine("16/06/2022 | 0 | 0");
        verify(spyConsole, times(1)).printLine("16/06/2023 | 0 | 0");
    }
    ...
    void should_print_accumulate_balance_along_with_date_later_when_call_print_given_date_casual_order() {
        Console spyConsole = mock(Console.class);
        Printer printer = new Printer(spyConsole);
        Transaction transaction1 = new Transaction("16/06/2019", 1);
        Transaction transaction2 = new Transaction("16/06/2018", 5);
        Transaction transaction3 = new Transaction("16/06/2017", 2);
        List<Transaction> givenTransactions = Arrays.asList(transaction1, transaction2, transaction3);

        printer.print(givenTransactions);

        verify(spyConsole, times(1)).printLine("16/06/2019 | 1 | 8");
        verify(spyConsole, times(1)).printLine("16/06/2018 | 5 | 3");
        verify(spyConsole, times(1)).printLine("16/06/2017 | 2 | 2");
    }
